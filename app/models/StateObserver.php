...
class StateObserver {
  ...
  public function created($model) {
  	...
  	if ($model->state=='ok' && State::whereHoldingId($model->holding_id)->andWhereState('annotated' )->exists() )
	  	State::whereHoldingId($model->holding_id)->andWhereState('annotated' )->delete();

  	//if state is 'annotated' delete ok state 
	if ($model->state=='annotated' && State::whereHoldingId($model->holding_id)->andWhereState('ok' )->exists())
