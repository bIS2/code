			...
			@if (!($HOSconfirm) || $HOSannotated)
				<td class="actions" holding="{{ $holding -> id }}">
					@if (!($HOSconfirm) && !($HOSincorrect))
				      ...
				      <a href="{{ url( 'sets/recall-holdings/'.$holding->id ) }}" set="{{$holdingsset->id}}" data-target="#modal-show" data-toggle="modal" class="pop-over" data-content="<strong>{{ trans('holdingssets.recall_hos_from_this_holding') }}</strong>" data-placement="top" data-toggle="popover" data-html="true" data-trigger="hover"><span class="fa fa-crosshairs"></span></a>
			...
			@endif
				<?php $k = 0; ?>
					@foreach ($fieldstoshow as $field)
						@if ($field != 'ocrr_ptrn')  
						<?php $k++;
							$field = (($field != 'exists_online') && ($field != 'is_current') && ($field != 'has_incomplete_vols')) ? $field = 'f'.$field : $field; 
						?>						
							<td>
								<?php 
