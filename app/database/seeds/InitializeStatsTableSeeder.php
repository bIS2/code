...
class InitializeStatsTableSeeder extends Seeder {
	...
	public function run()
	{
		...
		$initializestats = array(
			...
			'sets_annotated'					=> Holdingsset::annotated()->count(),
		...
		foreach ( Library::all() as $library) {
			...
			$stats_libraries = [
				'holdings_confirmed'				=> Holding::inLibrary($library->id)->confirms()->count(),
				'holdings_annotated'				=> Holding::inLibrary($library->id)->whereState('annotated')->count(),
				'holdings_ok'								=> Holding::inLibrary($library->id)->whereState('ok')->count(),
				'holdings_revised_ok'				=> Holding::inLibrary($library->id)->whereState('revised_ok')->count(),
				'holdings_revised_annotated'=> Holding::inLibrary($library->id)->whereState('revised_annotated')->count(),
